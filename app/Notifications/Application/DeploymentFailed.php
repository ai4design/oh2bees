...
class DeploymentFailed extends Notification implements ShouldQueue
{
    ...
    public function toMail(): MailMessage
    {
        ...
        if ($pull_request_id === 0) {
            $mail->subject('Oh2Bees: Deployment failed of '.$this->application_name.'.');
        } else {
            $fqdn = $this->preview->fqdn;
            $mail->subject('Oh2Bees: Deployment failed of pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.'.');
        }
    ...
    {
        if ($this->preview) {
            $message = 'Oh2Bees:  Pull request #'.$this->preview->pull_request_id.' of '.$this->application_name.' ('.$this->preview->fqdn.') deployment failed: ';
            $message .= '[View Deployment Logs]('.$this->deployment_url.')';
        } else {
            $message = 'Oh2Bees: Deployment failed of '.$this->application_name.' ('.$this->fqdn.'): ';
            $message .= '[View Deployment Logs]('.$this->deployment_url.')';
        }
