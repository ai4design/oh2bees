...
class TaskFailed extends Notification implements ShouldQueue
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("Oh2Bees: [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        return "Oh2Bees: Scheduled task ({$this->task->name}, [link]({$this->url})) failed with output: {$this->output}";
    }

    public function toTelegram(): array
    {
        $message = "Oh2Bees: Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in Oh2Bees',
                'url' => (string) $this->url,
            ];
        }
