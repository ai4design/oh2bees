        ...
        try {
            ...
            switch ($type) {
                ...
                case 'invoice.payment_failed':
                    ...
                    $subscription = Subscription::where('stripe_customer_id', $customerId)->first();
                    if (! $subscription) {
                        // send_internal_notification('invoice.payment_failed failed but no subscription found in Oh2Bees for customer: '.$customerId);

                        return response('No subscription found in Oh2Bees.');
                    }
                    $team = data_get($subscription, 'team');
                    if (! $team) {
                        // send_internal_notification('invoice.payment_failed failed but no team found in Oh2Bees for customer: '.$customerId);

                        return response('No team found in Oh2Bees.');
                    }
                ...
                case 'payment_intent.payment_failed':
                    ...
                    $subscription = Subscription::where('stripe_customer_id', $customerId)->first();
                    if (! $subscription) {
                        // send_internal_notification('payment_intent.payment_failed, no subscription found in Oh2Bees for customer: '.$customerId);
